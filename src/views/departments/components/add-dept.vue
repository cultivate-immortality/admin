...
export default {
  ...
  methods: {
    ...
    async getEmployeeSimple() {
      this.peoples = await getEmployeeSimple() // 获取员工简单列表
    },
      ...
      this.$refs.deptForm.validate(async isOK => {
        if (isOK) {
          if (this.formData.id) {
            // 要分清楚现在是编辑还是新增
            // 编辑模式 调用编辑接口
            await updateDepartments(this.formData) // 根据id修改部门详情
          } else {
            // 新增模式
    ...
    btnCancel() {
      ...
      this.$emit('update:showDialog', false) // 关闭添加子部门弹层
    },

    // 获取部门详情
    async getDepartDetail(id) {
      this.formData = await getDepartDetail(id) // 根据id查询部门详情
    }
  }
}
</script>
